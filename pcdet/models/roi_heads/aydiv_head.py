...
from .aydiv_head_utils import VGA,GCFAT,Sparse_Attention,Attention

class AYDIVHead(RoIHeadTemplate):
    def __init__(self, input_channels, model_cfg, point_cloud_range, voxel_size, num_class=1, **kwargs):
        ...
        self.dif = GCFAT(self.model_cfg.GCFAT[0],self.model_cfg.GCFAT[0],512,self.model_cfg.GCFAT[1])
        self.lmsa = Sparse_Attention(self.model_cfg.SFFA[0],self.model_cfg.SFFA[0])
        self.fusion_layer = VGA(c_out, c1, self.model_cfg.ATTENTION_FC[0])
    ...
    def forward(self, batch_dict):
        ...
        points_features_expand = self.Attention(points_features, points_neighbor)[1:]
        ...
        x_valid = self.sffa(x_pseudo,batch_dict['pseudo_features'] )
