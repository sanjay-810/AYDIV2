...
from ...utils import box_utils, calibration_waymo, common_utils, object3d_waymo
from ..dataset import DatasetTemplate

from collections import defaultdict
from pathlib import Path

class waymoDatasetAYDIV(DatasetTemplate):
    ...
    def get_label(self, idx):
        ...
        return object3d_waymo.get_objects_from_label(label_file)
    ...
    def create_groundtruth_database(self, info_path=None, used_classes=None, split='train'):
        ...
        db_info_save_path = Path(self.root_path) / ('waymo_dbinfos_%s_AYDIV_seguv.pkl' % split)
    ...
    def generate_prediction_dicts(batch_dict, pred_dicts, class_names, output_path=None):
        ...
        def generate_single_sample_dict(batch_index, box_dict):
            ...
            pred_boxes_camera = box_utils.boxes3d_lidar_to_waymo_camera(pred_boxes, calib)
            pred_boxes_img = box_utils.boxes3d_waymo_camera_to_imageboxes(
    ...
    def evaluation(self, det_annos, class_names, **kwargs):
        if 'annos' not in self.waymo_infos[0].keys():
            return None, {}

        from .waymo_object_eval_python import eval as waymo_eval

        eval_det_annos = copy.deepcopy(det_annos)
        eval_gt_annos = [copy.deepcopy(info['annos']) for info in self.waymo_infos]
        ap_result_str, ap_dict = waymo_eval.get_official_eval_result(eval_gt_annos, eval_det_annos, class_names)
    ...
    def __getitem__(self, index):
        ...
        if 'annos' in info:
            ...
            gt_boxes_lidar = box_utils.boxes3d_waymo_camera_to_lidar(gt_boxes_camera, calib)
...
def create_waymo_infos(dataset_cfg, class_names, data_path, save_path, workers=4):
    dataset = waymoDatasetAYDIV(dataset_cfg=dataset_cfg, class_names=class_names, root_path=data_path, training=False)
    ...
    waymo_infos_train = dataset.get_infos(num_workers=workers, has_label=True, count_inside_pts=True)
    with open(train_filename, 'wb') as f:
        pickle.dump(waymo_infos_train, f)
    print('waymo info train file is saved to %s' % train_filename)

    dataset.set_split(val_split)
    waymo_infos_val = dataset.get_infos(num_workers=workers, has_label=True, count_inside_pts=True)
    ...
    waymo_infos_test = dataset.get_infos(num_workers=workers, has_label=False, count_inside_pts=False)
...
if __name__ == '__main__':
    ...
    if sys.argv.__len__() > 1 and sys.argv[1] == 'create_waymo_infos':
        ...
        ROOT_DIR = Path('../data/waymo_AYDIV_seguv_twise')
        create_waymo_infos(
