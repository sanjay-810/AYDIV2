...
def evaluate(label_path,
             ...
             coco=False,
             score_thresh=-1):
    dt_annos = waymo.get_label_annos(result_path)
    if score_thresh > 0:
        dt_annos = waymo.filter_annos_low_score(dt_annos, score_thresh)
    val_image_ids = _read_imageset_file(label_split_file)
    gt_annos = waymo.get_label_annos(label_path, val_image_ids)
    if coco:
