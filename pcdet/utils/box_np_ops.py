...
def center_to_corner_box3d(centers,
                           ...
                           origin=(0.5, 1.0, 0.5),
                           axis=1):
    """Convert waymo locations, dimensions and angles to corners.

    Args:
        centers (np.ndarray): Locations in waymo label file with shape (N, 3).
        dims (np.ndarray): Dimensions in waymo label file with shape (N, 3).
        angles (np.ndarray): Rotation_y in waymo label file with shape (N).
        origin (list or array or float): Origin point relate to smallest point.
    ...
    Returns:
        np.ndarray: Corners with the shape of (N, 8, 3).
          ...
          3 -------- 0
    """
    # 'length' in waymo format is in x axis.
    # yzx(hwl)(waymo label file)<->xyz(lhw)(camera)<->z(-x)(-y)(wlh)(lidar)
    # center in waymo format is [0.5, 1.0, 0.5] in xyz.
